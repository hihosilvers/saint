        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if "cutmix" in opt.pt_aug:
                ...
                x_categ_corr, x_cont_corr = add_noise(
                    x_categ, x_cont, noise_params=pt_aug_dict
                )
                _, x_categ_enc_2, x_cont_enc_2 = embed_data_mask(
                    x_categ_corr, x_cont_corr, cat_mask, con_mask, model, vision_dset
                )
            else:
                _, x_categ_enc_2, x_cont_enc_2 = embed_data_mask(
            ...
            if "mixup" in opt.pt_aug:
                ...
                x_categ_enc_2, x_cont_enc_2 = mixup_data(
                    x_categ_enc_2, x_cont_enc_2, lam=opt.mixup_lam
                )
            loss = 0
            if "contrastive" in opt.pt_tasks:
                ...
                aug_features_1 = (
                    aug_features_1 / aug_features_1.norm(dim=-1, keepdim=True)
                ).flatten(1, 2)
                aug_features_2 = (
                    aug_features_2 / aug_features_2.norm(dim=-1, keepdim=True)
                ).flatten(1, 2)
                ...
                targets = torch.arange(logits_per_aug1.size(0)).to(
                    logits_per_aug1.device
                )
            ...
            if "denoising" in opt.pt_tasks:
                ...
                if len(con_outs) > 0:
                    if not isinstance(con_outs, torch.Tensor):
                        con_outs = torch.cat(con_outs, dim=1)
                    else:
                        con_outs = torch.squeeze(con_outs)
